...
class spech_search_through_directory:
    ...
    def __init__(self, audio_path = None,directoryPath = None, saving_path = None, current_path = None, wav2vec_model = None, 
        ...
        processed_audios_names = [f.replace('_scores', '').replace('csv', 'wav') for f in processed_audios_names if f.endswith('csv')]
        ...
        processed_audios_names3 = [f.replace('_scores', '').replace('csv', 'wav') for f in processed_audios_names3 if f.endswith('csv')]
    ...
    def get_sgements_with_high_score(self, file_name, folder_path = None, saving_folder = None, queries = ['read books', 'watch movies'], thresh_score = .06, audio_length = 30  ):
        ...
        try:
            os.makedirs(os.path.join(saving_folder, 'result')) # creating a folder named resutls for saving .wav files with acceptable score
        except:
            pass
        ...
        for i, chunk in enumerate(chunks):
            ...
            search = spech_search(file_path = chunk_name_for_analysis, wav2vec_model = self.wav2vec_model, wav2vec_tokenizer = self.wav2vec_tokenizer)
            transcription = search.get_transcription()
            semantic_search_via_audio = semantic_search_for_audio(transcription, model = self.semantic_model, tokenizer = self.semantic_tokonizer)
    ...
    def run_through_all_files(self, running_path = None, queries = ['read books', 'watch movies'] ):
        ...
        try:
            ...
            config = json.load(open(os.path.join(running_path, "config.txt")))   # audio_legnth  queries    score_threshold
        ...
        for each_file in self.all_wav_files:
                ...
                self.get_sgements_with_high_score(each_file, self.directoryPath , self.saving_path, queries = queries, thresh_score = thresh_score, audio_length = audio_length  )
