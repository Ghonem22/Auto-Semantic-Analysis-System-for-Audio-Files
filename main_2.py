    ...
    while True:
        ...
        if now.minute % 1 == 0 :
            try:
                ...
                spech_search_instance = spech_search_through_directory(audio_path = 'drive1/new_audio_server/audio_search/data/', directoryPath = 'data2/', saving_path = 'data2/', wav2vec_model = wav2vec_model, wav2vec_tokenizer = wav2vec_tokenizer, semantic_model = semantic_model, semantic_tokonizer = semantic_tokonizer, f = 6, i = 2)
                spech_search_instance.run_through_all_files(queries = ['read books'] )
                spech_search_instance.after_process()
